package com.base.library.view.loading;
...
public class CellularView extends SurfaceView {
    private static final String TAG = "CellularView";
    //view的宽度和高度
    ...
    //是否自动开始动画
    private boolean autoStartAnim = true;
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = getTypedArray(context, attributeSet, R.styleable.CellularView);
        if (attr == null) {
            return;
        }
        try {
            color = attr.getColor(R.styleable.CellularView_CellularColor, ContextCompat.getColor(context, R.color.top_color));//默认橙色
        } finally {
            attr.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (viewWidth == 0) {
            viewWidth = CodeUtil.dip2px(getContext(), 100);
            viewHeight = CodeUtil.dip2px(getContext(), 100);
        }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        WDYLog.d(TAG, "onWindowFocusChanged  " + hasWindowFocus);
