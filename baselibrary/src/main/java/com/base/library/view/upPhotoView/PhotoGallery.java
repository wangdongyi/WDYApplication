...
public class PhotoGallery extends Gallery {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (view instanceof MyImageView) {
            ...
            float min_distance = HotPictureViewActivity.screenWidth / 4f;
            ...
            if ((int) width <= HotPictureViewActivity.screenWidth && (int) height <= HotPictureViewActivity.screenHeight)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
            {
                ...
                if (distanceX > 0)// 向左滑动
                {
                    if (r.left > 0 || right < HotPictureViewActivity.screenWidth) {// 判断当前ImageView是否显示完全
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                ...
                {
                    if (r.right < HotPictureViewActivity.screenWidth || left > 0) {
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                        imageView.postTranslate(-distanceX, -distanceY);
                    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if (view instanceof MyImageView) {
                    ...
                    if ((int) width <= HotPictureViewActivity.screenWidth && (int) height <= HotPictureViewActivity.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
                    {
                        break;
                    }
                    float v[] = new float[9];
                    ...
                    if (top < 0 && bottom < HotPictureViewActivity.screenHeight) {
                        // imageView.postTranslateDur(-top, 200f);
                        imageView.postTranslateDur(HotPictureViewActivity.screenHeight - bottom, 200f);
                    }
                    if (top > 0 && bottom > HotPictureViewActivity.screenHeight) {
                    ...
                    if (left < 0 && right < HotPictureViewActivity.screenWidth) {
                        // imageView.postTranslateXDur(-left, 200f);
                        imageView.postTranslateXDur(HotPictureViewActivity.screenWidth - right, 200f);
                    }
                    if (left > 0 && right > HotPictureViewActivity.screenWidth) {
