...
public class CropImage {
    ...
    public CropImage(Context context, CropImageView imageView, Handler handler){
        ...
        mHandler = handler;
        //初始化图片保存路径
        FILE_LOCAL = new File("sdcard/image/");
    ...
    public String saveToLocal(Bitmap bitmap){
        //需要裁剪后保存为新图片
        String mFileName = new Random().nextInt(999999) + ".jpg";
    ...
    public class BackgroundJob implements Runnable {
        ...
        public void run(){
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    try{
                        mHandler.sendMessage(mHandler.obtainMessage(DoPicCapUtil.SHOW_PROGRESS));
                    }catch (Exception e) {
                    }
                    latch.countDown();
                }
            ...
            try {
                ...
                mHandler.sendMessage(mHandler.obtainMessage(DoPicCapUtil.REMOVE_PROGRESS));
            }
        }
    }
}
